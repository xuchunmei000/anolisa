...
Skill Ledger 不提供面向用户或 SkillFS 的 `resolve` CLI；activation refresh 是 daemon 内部职责。resolver 根据当前版本链和默认策略 `pass_only` 选择可运行 snapshot，并原子写入 `.skill-meta/activation.json`，同时尽力同步写入 skill 目录 xattr `user.agent_sec.skill_ledger.activation`：
...
若没有签名有效、manifest hash 有效、snapshot 完整且 `scanStatus=pass` 的版本，则写入：
...
resolver 始终只激活 snapshot，不激活 source/current 工作区。当前工作区处于 `drifted`、`warn`、`deny` 或 `tampered` 时，会继续指向最近可信 pass snapshot；若不存在可信版本，则 `target=null`。daemon 在收到 SkillFS 变更通知、扫描完成或重启 reconcile 时调用该 resolver。
